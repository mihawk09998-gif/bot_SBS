...
DEFAULT_DATA = {
    ...
    "faq": [
        ...
        },
        {
            ...
            "answer": "✍️ Нажмите кнопку «✍️ Записаться» в меню ниже, чтобы быстро записаться на пробное занятие или курс! 😊"
        }
    ]
}
...
7. Не отправляй клиента звонить или писать менеджеру/директору по телефону или в WhatsApp. Вместо этого активно побуждай нажимать на кнопки меню: для записи на пробное занятие или курс подталкивай нажать кнопку «✍️ Записаться», а для просмотра всех курсов — «📚 Наши курсы». Предлагай контакты центра (+996 701 000 712 или 0505091285) только в самом крайнем случае, если клиент прямо и настойчиво просит номер телефона.
...
async def handle_ai_query(message: types.Message, state: FSMContext):
    ...
    if not client:
        return await message.answer(
            "Вы можете узнать подробности или записаться с помощью кнопок меню ниже (например, нажмите «📚 Наши курсы» или «✍️ Записаться»)! 😊",
            reply_markup=get_main_keyboard()
        )
    ...
    try:
        ...
        await message.answer(
            "Извините, произошла техническая ошибка. Пожалуйста, воспользуйтесь кнопками меню ниже (например, нажмите «📚 Наши курсы» или «✍️ Записаться»), чтобы узнать подробности или записаться! 😊",
            reply_markup=get_main_keyboard()
        )
